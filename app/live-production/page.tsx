...
export default function LiveProduction(){
  ...
  return <LegacyShell>
    ...
    <section className="live-summary">
      <Card className="live-kpi target-kpi"><Target/><div><span>Daily Target</span><strong>{target}</strong><small>orders planned for today</small></div></Card>
      <Link className="live-kpi live-kpi-link production-kpi" href="/?view=in-production#orders-table"><Card><Activity/><div><span>In Production</span><strong>{inProduction}</strong><small>orders currently in progress</small></div></Card></Link>
      <Link className="live-kpi live-kpi-link completed-kpi" href="/?stage=Finished&view=completed-today#orders-table"><Card><CheckCircle2/><div><span>Completed Today</span><strong>{completedToday}</strong><small>orders finished today</small></div></Card></Link>
    ...
    <Card className="flow-card live-production-card"><div className="section-heading"><div><h2>Production Flow</h2><p>Click a station to view its orders on the Dashboard.</p></div><button className="live-today" type="button"><CalendarDays/>Today<ChevronDown/></button></div><Flow/></Card>
  </LegacyShell>
}
